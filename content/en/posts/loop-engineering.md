...
So this seemed kind of optimal. Loops which tried to automatize more just seemed to scary and my prior experience would just tell me they would converge into slop. But I still decided to look into them.

## What is Loop Engineering

Loop engineering is the discipline of engineering the feedback loop that wraps the agent.

- The main goal is to be able to merge the code to the repo without human supervision needed.

So the delegation part and the verification part was handled by me should be handled by an agent.
...
So are there solutions to these problems? Well partly and there are some cool insight I gathered along the way that could even be helpful to the main workflow I'm using on the way.
...
So you could use a weak verifier and fresh context to evaluate a certain feature, given that it can execute a script which tests the certain flow of the feature. This verified script should be the ground truth and the agent can judge its outcome quite easily. The intellectual load and the design of the verifier is in control of the loop engineer. To make sure to avoid cheating of the coding agent you should of course strictly prohibit it from changing the verifier script.
...
The sources I have used while writing this post:

- **The Kitchen Loop: User-Spec-Driven Development for a Self-Evolving Codebase** — [arxiv.org/abs/2603.25697](https://arxiv.org/abs/2603.25697)
- **Loop Engineering: Four Loops That Actually Works** — zostaff — [x.com/zostaff/status/2070852153594290195](https://x.com/zostaff/status/2070852153594290195)
- **Loop Engineering: The Anthropic Playbook for Designing Systems That Prompt Your Agents** — [drive.google.com](https://drive.google.com/file/d/1qzKI4DKnyHRpXK1J3ATPqwaqLc0iNu-M/view)
